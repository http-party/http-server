...
function safeResolvePath(root, requestPath) {
  const rootResolved = path.resolve(root);
  const relativeRequestPath = String(requestPath || '').replace(/^[/\\]+/, '');
  const resolved = path.resolve(rootResolved, relativeRequestPath);
...
function decodePathname(pathname) {
  ...
  const normalized = path.normalize(pieces.map((rawPiece) => {
    ...
    if (piece.includes('\0')) {
      throw new Error('Invalid null byte character');
    }
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  return function middleware(req, res, next) {
    ...
    const relativePath = path.relative(path.join('/', opts.baseDir), pathname);
    file = safeResolvePath(root, relativePath);
