...
function HttpServer(options) {
  ...
  // hack for application/wasm while using ecstatic 3.x (need to remove '; charset' from Content-Type header)
  function overwriteSetHeader(orig) {
      return function(header, val) {
        if (header == 'content-type' || header == 'Content-Type') {
          if (val.indexOf('application/wasm') != -1) {
            val = 'application/wasm'
          }
          console.log(`header: ${header}, val: ${val}`)
        }
        orig.call(this, header, val);
      }
  }

  before.push(function (req, res) {
    const orig = res.setHeader;    
    res.setHeader = overwriteSetHeader(orig).bind(res);      
    ecstaticMiddleWare(req, res);   
  });
